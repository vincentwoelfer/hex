extends Node3D
class_name VFXAoeRangeIndicator

static var scene: PackedScene = preload("res://scenes/effects/AoeRangeIndicator.tscn")

static func spawn_global_pos(pos: Vector3, radius: float, lifetime: float) -> VFXAoeRangeIndicator:
	var instance := scene.instantiate() as VFXAoeRangeIndicator
...
static func spawn_at_parent(parent: Node3D, radius: float, lifetime: float) -> VFXAoeRangeIndicator:
	var instance := scene.instantiate() as VFXAoeRangeIndicator
	instance._resize_decal(radius)
