...
# See https://github.com/matjlars/godot-multiplayer-input/blob/main/demo/player_manager.gd
...
# the purpose of this is to cache all the StringNames of all the actions so it doesn't need to generate them every time
var device_actions: Dictionary[int, Dictionary] = {}

## Array of GUIDs - If a device with an ignored GUID is detected, no input actions will be added.
var ignored_guids: Array[int] = []
const MAX_DEVICES: int = 8

# Devices are numbered from 0 to MAX_DEVICES - 1
# -1 for keyboard, 0 for first controller, ...
...
# Call this if you change any of the core actions or need to _reset everything.
func _reset() -> void:
	InputMap.load_from_project_settings()
	core_actions = InputMap.get_actions()

	# disable joypad events on keyboard actions
	# by setting device id to MAX_DEVICES (out of range, so they'll never trigger)
...
func _create_actions_for_device(device: int) -> void:
	...
	for core_action in core_actions:
		var new_action: StringName = "%s%s" % [device, core_action]
		var deadzone: float = InputMap.action_get_deadzone(core_action)

		# get all joypad events for this action
		var events: Array[InputEvent] = InputMap.action_get_events(core_action).filter(_is_joypad_event)
...
func _delete_actions_for_device(device: int) -> void:
	print("Deleting actions for device %s" % device)

	device_actions.erase(device)
	var actions_to_erase: Array[String] = []
...
## Pass a -2 to _reset it back to default behavior, to allow all devices to trigger "ui_" actions.
## For example, pass a -1 if you want only the keyboard/mouse device to control menus.
## NOTE: this calls _reset(), so if you make any changes to the InputMap via code, you'll need to make them again.
func set_ui_action_device(device: int) -> void:
	print("Setting UI action device to %s" % device)
...
func is_ui_action(action_name: StringName) -> bool:
	return action_name.begins_with("ui_")

func is_cam_action(action_name: StringName) -> bool:
	return action_name.begins_with("cam_")
