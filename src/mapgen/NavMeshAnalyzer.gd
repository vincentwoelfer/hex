...
func _analyze() -> void:
	...
	for i in range(polygon_count):
		...
		var is_inside_geometry := PhysicUtil.point_collision_test(collision_pos + Vector3(0, 0.3, 0), Layers.PHY_TERRAIN_AND_STATIC)
...
func _is_on_terrain(cluster_vertex_indices: PackedInt32Array) -> bool:
	...
	for f: float in checks_at:
		...
		var hit := PhysicUtil.raycast_test(collision_pos + offset, collision_pos - offset, Layers.mask([Layers.PHY.TERRAIN]))
		if hit:
			is_on_terrain = true
			break
