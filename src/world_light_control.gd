...
var hours_duration: float = 1.0 # in seconds
...
@export var min_light_energy: float = 0.5 # Turned off currently
...
func _process(delta: float) -> void:
	...
	var new_hour := floorf(world_time)
	if current_hour != new_hour:
		current_hour = new_hour as int
		update_lighting(world_time, current_weather)
...
func update_lighting(time: float, weather: WeatherType) -> void:
	...
	for property: String in sun_properties.keys():
		tween.tween_property(sun, property, sun_properties[property], current_tween_duration)

	var sky_mat := world_environment.environment.sky.sky_material as PanoramaSkyMaterial
	#sky_mat.energy_multiplier = sun_properties['light_energy']
	tween.tween_property(sky_mat, 'energy_multiplier', sun_properties['sky_light_energy'], current_tween_duration)
...
func get_day_time_properties(time) -> Dictionary:
	...
	# ToDo correctly set light_energy to min_light_energy during night
	# Turn sun of
	var light_energy = 0
	var sky_light_energy = 0.5
